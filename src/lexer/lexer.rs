...
use crate::{
    ...
    file_manager::{file::FileId, sm::SourceManager, span::{PosIndex, Span}},
};
use core::f64;
...
impl Lexer {
    ...
    pub fn lex_code(&mut self, id: FileId, sm: &SourceManager, posindx: &PosIndex) -> Result<Vec<Token>, Error> {
        ...
        while pos < code.len() {
            ...
            if self.is_string {
                best_match = Some((
                    self.rules.iter().find(|r| r.name == consts::STR).unwrap(),
            ...
            if let Some((rule, m)) = best_match {
                ...
                match rule.name {
                    ...
                    consts::WHITESPACE => {
                        tokens.push(self.gen_token(TokenType::WHITESPACE));
                    }
